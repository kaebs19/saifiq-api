...
const parseRequired = (envName, fallback) => {
  const v = parseInt(process.env[envName], 10);
  return Number.isFinite(v) && v >= 2 ? v : fallback;
};

const REQUIRED = {
  [GAME_MODES.ONE_V_ONE]: parseRequired('REQUIRED_1V1', 2),
  [GAME_MODES.FOUR_PLAYER]: parseRequired('REQUIRED_4PLAYER', 4),
};

const ALL_QUEUE_KEYS = Object.keys(REQUIRED).map(QUEUE_KEY);

const sweepStale = async (mode) => {
  const cutoff = Date.now() - STALE_QUEUE_MS;
  await redis.zremrangebyscore(QUEUE_KEY(mode), 0, cutoff);
};
...
const leaveQueue = async (userId) => {
  ...
  // Fallback: helper key may have expired \u2014 scrub user from every known queue
  await Promise.all(ALL_QUEUE_KEYS.map((k) => redis.zrem(k, userId)));
};
